...
export function MapControls({ onLocateMe, onZoomIn, onZoomOut, currentZoom, maxMapZoom }: MapControlsProps) {
  return (
    <div className="absolute bottom-4 right-3 z-20 flex flex-col items-center gap-2 md:bottom-7 md:right-6 md:gap-3">
      <button type="button" className="grid h-11 w-11 place-items-center rounded-full bg-white text-red-500 shadow-lg md:h-14 md:w-14" aria-label="Report">
      ...
      <button type="button" className="grid h-11 w-11 place-items-center rounded-full bg-white text-slate-700 shadow-lg md:h-14 md:w-14" aria-label="Locate me" onClick={onLocateMe}>
        <svg viewBox="0 0 24 24" className="h-8 w-8" fill="none" stroke="currentColor" strokeWidth="2.2">
      ...
      <div className="overflow-hidden rounded-2xl bg-white shadow-lg">
        ...
        <button type="button" className="grid h-11 w-11 place-items-center text-4xl text-slate-700 transition hover:bg-slate-100 md:h-14 md:w-14 md:text-5xl" aria-label="Zoom out" onClick={onZoomOut}>
          -
        </button>
      </div>
    </div>
